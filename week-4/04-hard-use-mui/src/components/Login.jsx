...
function Login() {
    ...
    async function login() {
        ...
        try {
            const response = await axios.post('http://localhost:3000/admin/login',input,{headers});
    ...
    <div>
        <Box sx = {{marginTop: "3rem", marginLeft: "32rem"}}>
            <h1 style = {{fontFamily: "monospace"}}>Sign in</h1>
            <Text >Don't have an account? <a href = '/register' style={{textDecorationLine: "none",color: "blue"}}>Create one</a></Text>
        </Box>
        <Flex direction={"column"} justify={"center"} align={"center"} sx = {{marginTop: "2rem",marginRight: "1rem"}} gap={"0.5rem"} >
            ...
            <button onClick={login} style = {{marginTop: "2rem", width: "31vw",padding: "0.8rem 0.5rem",backgroundColor: "darkorange",border: "none", borderRadius: "5px",fontSize: "1.2rem",color: "white",cursor: "pointer"}}>Login</button>
        </Flex>
